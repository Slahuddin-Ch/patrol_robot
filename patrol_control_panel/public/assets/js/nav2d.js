...
NAV2D.Navigator = function(options) {
  ...
  function sendGoal(pose) {
    ...
    var child_length = that.rootObject.children.length;
    if (child_length > 3){
      that.rootObject.removeChild(goalMarker);
    }
    that.rootObject.addChild(goalMarker);

    NAV2D.delete_marker = function (){
      if (child_length > 2){
      that.rootObject.removeChild(goalMarker);
      }
    }
  ...
  if (withOrientation === false){
    ...
    var mouseEventHandler = function(event, mouseState) {
      ...
      if (mouseState === 'down'){
        ...
        NAV2D.rootObject.removeChild(goalMarker);
        sendGoal(pose);
        NAV2D.goal_pose = pose;
      }
    };
    ...
    NAV2D.enable_goal = function (){
    this.rootObject.addEventListener('stagemousedown', mouse_down);

    this.rootObject.addEventListener('stagemousemove', mouse_move);

    this.rootObject.addEventListener('stagemouseup', mouse_up);
  }

  NAV2D.disable_goal = function (){
    this.rootObject.removeEventListener('stagemousedown',mouse_down);
