...
ROS2D.OccupancyGridClient = function(options) {
  ...
  this.addMap(this.topic);
}
ROS2D.OccupancyGridClient.prototype.addMap = function(topic){
  var that = this;
  // subscribe to the topic
  var rosTopic = new ROSLIB.Topic({
    ros : this.ros,
...
//	var that = this;
	...
	this.fillColor = options.pointColor || createjs.Graphics.getRGB(247, 119, 123, 0.33);
 ...
 */
ROS2D.PolygonMarker.prototype.remPoint = function(obj,delete_all) {
	var index;
//	var point;
...
ROS2D.PolygonMarker.prototype.drawFill = function() {
    ...
    g.lineTo(this.pointContainer.getChildAt(0).x, this.pointContainer.getChildAt(0).y);
		g.closePath();
		g.endFill();
		g.endStroke();
	}
...
//	var that = this;
	...
	this.strokeSize = options.strokeSize || 3;
	this.strokeColor = options.strokeColor || createjs.Graphics.getRGB(228, 0, 0);
...
ROS2D.TraceShape.prototype.addPose = function(pose) {
  //console.log("TRACE SHAPE CALLED",pose.position.x)
...
var zoom= 1.0;
var zoom = false;
ROS2D.ZoomView.prototype.zoom = function(value) {
  if (count == 1){
    initX = this.stage.x;
    initY = this.stage.y;
  }
  if (count == 2){
    initScaleX = this.stage.scaleX;
    initScaleY = this.stage.scaleY;
    }
  count += 1;
  ...
  this.stage.y = this.startShift.y - (this.center.y-this.startShift.y) * (this.stage.scaleY/this.startScale.y - 1);
    ...
    if (this.stage.scaleX > initScaleX){
    zoom -= 0.02;
    this.stage.scaleX = this.startScale.x*zoom;
    this.stage.scaleY = this.startScale.y*zoom;
    this.stage.x = this.startShift.x - (this.center.x-this.startShift.x) * (this.stage.scaleX/this.startScale.x - 1);
  this.stage.y = this.startShift.y - (this.center.y-this.startShift.y) * (this.stage.scaleY/this.startScale.y - 1);
      ...
      this.stage.scaleX = initScaleX;
      this.stage.scaleY = initScaleY;
      this.stage.x = initX;
      this.stage.y = initY;
    }
  }
